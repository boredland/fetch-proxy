...
```
- `?auto=1` — **auto-escalate** when a response looks blocked: plain fetch → FlareSolverr → stealth render, returning the first tier that isn't blocked (falls back to the real upstream response if none succeed). "Blocked" = a `401`/`403`/`429`/`503`, a page of any size carrying a Cloudflare challenge fingerprint (the managed-challenge variant runs ~90 KB with its marker at the very end), or a small page matching a known anti-bot wall (Akamai, PerimeterX, Incapsula, captcha/JS walls). `401` counts because some origins refuse the proxy's egress IP with it rather than a `403`. The small-page limit applies only to the generic vendor wording — "access denied", "recaptcha" and the like are ordinary English that a real article can legitimately contain. Set `AUTO_FALLBACK=1` to make this the default and use `?auto=0` to opt out per request.
...
- `?readability=0` — with `format=md`, skip Readability's main-content extraction and convert the **whole body** (minus scripts/styles and consent banners). Readability optimises for *prose*: on a listing-shaped page — an event calendar, a course table, a price list — it succeeds and returns only the description, dropping the dates and figures around it. Measured on a theatre's event page, the extracted article is 782 bytes and contains **neither of the event's two dates**; body mode returns 5 KB including them. Larger output, but still ~10x smaller than the raw HTML, and the implicit size cap that guards the no-article fallback does not apply (an explicit body request is a choice, not a fallback).
